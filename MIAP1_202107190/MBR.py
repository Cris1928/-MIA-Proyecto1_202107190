...
class MBR:
    ...
    def pack(self):
        ...
        fit_char = self.fit[0].encode()  # aqui se codifica el fit
    
        packed_mbr = struct.pack(self.FORMAT, self.mbr_tamano,self.mbr_fecha_creacion, self.mbr_dsk_signature, fit_char) # aqui se codifica el mbr
        
        packed_objetos = b''.join([obj.pack() for obj in self.particiones]) # aqui se codifica las particiones
    ...
    def unpack(cls, data):
        ...
        ex = {'size': 5, 'path': 'path', 'name': 'part_temp'}
        ...
        ex = {'size': 10, 'path': 'path', 'name': 'part_temp'}
        #se crea una lista de 4 particiones
